    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-emerald-500 selection:text-slate-950">
      ...
      <main className="flex-1 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-10 text-xs sm:text-sm leading-relaxed text-slate-300">
        ...
        <section className="space-y-3">
          ...
          <p>
            {isEn
              ? 'Aria Prop ("the Service") is a B2B Software as a Service (SaaS) property sales assistant and CRM integration platform provided by [NOMBRE_DE_LA_EMPRESA_O_TITULAR] ("Company", "we", "us"). By registering, accessing or using the platform, you agree to be bound by these Terms of Service.'
              : isPt
              ? 'A Aria Prop ("o Serviço") é uma plataforma B2B de assistência comercial imobiliária e integração com CRMs fornecida por [NOMBRE_DE_LA_EMPRESA_O_TITULAR] ("Empresa", "nós"). Ao se registrar ou usar a plataforma, você concorda em cumprir estes Termos de Serviço.'
              : 'Aria Prop ("el Servicio") es una plataforma B2B de software como servicio (SaaS) para asistencia comercial inmobiliaria, cualificación de prospectos y sincronización de catálogos con CRMs, provista por [NOMBRE_DE_LA_EMPRESA_O_TITULAR] ("La Empresa", "nosotros"). Al registrarse, acceder o utilizar la plataforma, usted acepta quedar vinculado por estos Términos de Servicio.'}
          </p>
        </section>
        ...
        <section className="space-y-3 p-5 rounded-2xl bg-slate-900 border border-white/10">
          ...
          <ul className="space-y-1 text-slate-300 text-xs font-mono">
            <li>• <strong>{isEn ? 'Legal Holder / Company:' : isPt ? 'Razão Social:' : 'Razón Social / Titular Legal:'}</strong> [NOMBRE_DE_LA_EMPRESA_O_TITULAR]</li>
            <li>• <strong>{isEn ? 'Tax ID / CUIT:' : isPt ? 'ID Fiscal:' : 'Identificación Fiscal / CUIT:'}</strong> [NUMERO_CUIT_O_TAX_ID]</li>
            <li>• <strong>{isEn ? 'Address:' : isPt ? 'Endereço:' : 'Dirección Fiscal:'}</strong> [DIRECCION_FISCAL_COMPLETA]</li>
